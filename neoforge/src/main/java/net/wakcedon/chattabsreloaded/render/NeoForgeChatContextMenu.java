...
public class NeoForgeChatContextMenu {
    ...
    public NeoForgeChatContextMenu(int x, int y, Element... elements) {
        this.x = x;
        this.y = y;
    ...
    public void render(Minecraft client, GuiGraphicsExtractor context, int windowWidth, int windowHeight, int mouseX, int mouseY) {
        ...
        for(Element element : elements) {
            if(element.isDivider()) {
                ...
                context.text(client.font, element.text(), x + 2, ey + 2, -1, NeoForgeMod.getConfig().getConfig().textShadow);
                ey += ELEMENT_HEIGHT;
            }
        }
    }
