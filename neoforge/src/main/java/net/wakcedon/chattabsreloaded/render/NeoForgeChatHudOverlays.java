...
public class NeoForgeChatHudOverlays {
    ...
    public static int[] renderChatTabs(Minecraft client, int tabScroll, GuiGraphicsExtractor context, int windowHeight, float chatScale, boolean expanded, int chatWidth, int mouseX, int mouseY, int messages, boolean mcTabUnreads) {
        int hoveredTab = -1;
        if(!NeoForgeMod.getConfig().getConfig().enabled || NeoForgeMod.getConfig().getConfig().getVisibleChatTabs().isEmpty() || !expanded || chatScale == 0) return new int[]{hoveredTab, tabScroll};
        ...
        context.fill(x, y, x + width, y + height, hovered ? NeoForgeMod.getConfig().getConfig().bgColorHovered.getRGB() : NeoForgeMod.getConfig().getConfig().bgColor.getRGB());
        context.text(client.font, "MC", x + 2, y + 2, -1, NeoForgeMod.getConfig().getConfig().textShadow);
        if(NeoForgeMod.getConfig().getConfig().selectedTab == tabNum) {
            context.fill(x, y - 1, x + width, y, NeoForgeMod.getConfig().getConfig().selectedTabColor.getRGB());
        } else if(mcTabUnreads) {
            context.fill(x, y - 1, x + width, y, NeoForgeMod.getConfig().getConfig().unreadColor.getRGB());
        }
        if(hovered) {
            hoveredTab = tabNum;
        }
        ...
        if(tabScroll > -1) {
            ...
            context.fill(x, y, x + scrollerWidth, y + height, hovered ? NeoForgeMod.getConfig().getConfig().bgColorHovered.getRGB() : NeoForgeMod.getConfig().getConfig().bgColor.getRGB());
            context.text(client.font, "<", x + 2, y + 2, -1, NeoForgeMod.getConfig().getConfig().textShadow);
            if(hovered) {
                hoveredTab = -2;
            }
            x += scrollerWidth;
        ...
        for(ChatTab tab : NeoForgeMod.getConfig().getConfig().getVisibleChatTabs()) {
            ...
            if(x + width > chatWidth - scrollerWidth) {
                ...
                context.fill(chatWidth - scrollerWidth, y, chatWidth, y + height, hovered ? NeoForgeMod.getConfig().getConfig().bgColorHovered.getRGB() : NeoForgeMod.getConfig().getConfig().bgColor.getRGB());
                context.fill(x, y, chatWidth - scrollerWidth, y + height, NeoForgeMod.getConfig().getConfig().bgColor.getRGB());
                context.text(client.font, ">", chatWidth - scrollerWidth + 2, y + 2, -1, NeoForgeMod.getConfig().getConfig().textShadow);
                if(hovered) {
                    hoveredTab = -3;
                }
                break;
            ...
            context.fill(x, y, x + width, y + height, hovered ? NeoForgeMod.getConfig().getConfig().bgColorHovered.getRGB() : NeoForgeMod.getConfig().getConfig().bgColor.getRGB());
            context.text(client.font, tab.getName(), x + 2, y + 2, -1, NeoForgeMod.getConfig().getConfig().textShadow);
            if(NeoForgeMod.getConfig().getConfig().selectedTab == tabNum) {
                context.fill(x, y - 1, x + width, y, NeoForgeMod.getConfig().getConfig().selectedTabColor.getRGB());
